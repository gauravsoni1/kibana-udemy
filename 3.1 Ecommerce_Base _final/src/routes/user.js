...
const apm = require('../config/apm.config');
...
router.post("/signin", (req, res) => {
  ...
  if (!reqBody.username || !reqBody.password) {
    const invalidSignup = new Error("Invalid signup details");
    apm.captureError(invalidSignup, {
      custom: {
        user: "gaurav",
        email: "gaurav@gmail.com"
      }
    });
    return res.status(400).send(invalidSignup);
  }
